...
J-Jobs 컨테이너 이미지에는 `awscli` / `kubectl` 이 포함되어 있어, OS/kubectl 단에서 인증이 동작하면 IRSA / kubeconfig Secret / 정적 자격증명 등 어떤 방식이든 사용할 수 있다. 다중 계정 EKS 환경에서는 아래 IRSA + AssumeRole 흐름을 권장한다.
