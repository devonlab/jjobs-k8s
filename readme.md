...
- `.spec.template.spec.containers.lifecycle.preStop` : 컨테이너가 종료되기 직전 호출되는 명령어로, 매니저/서버/Agent가 권장 상태로 종료되도록 한다. J-Jobs는 이를 위한 참조 구현으로 `pre-stop.sh`를 이미지에 포함해 제공하며(위 예시 참고), 래핑 이미지나 커스텀 스크립트로 직접 구성해도 된다. 단, Job 실행 정보 정합성을 위해 graceful stop 동작은 동등하게 보장해야 한다.
- `.spec.template.spec.containers.lifecycle.postStart` : 컨테이너가 생성된 직후 호출되는 명령어로, 서비스 정상 기동 확인 및 일시정지된 서버/에이전트를 재개한다. J-Jobs는 이를 위한 참조 구현으로 `post-start.sh`를 이미지에 포함해 제공하며(위 예시 참고), 래핑 이미지나 커스텀 스크립트로 직접 구성해도 된다.
