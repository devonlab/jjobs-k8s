...
## 다중 EKS 클러스터(Multi-Cluster) 가이드

J-Jobs Agent가 설치된 EKS 클러스터가 아닌 **다른 계정/다른 EKS 클러스터에 Job Pod를 생성**해야 하는 시나리오를 위한 구성 가이드이다. K8sJob 플러그인은 모든 `kubectl` 명령에 `--context=<alias>` 플래그를 부착하므로, Agent 컨테이너 내부에 대상 클러스터의 kubectl context가 등록되어 있고 해당 context로 인증이 동작하는 환경이라면 어떤 인증 방식이든 사용 가능하다.

### 책임 분담

J-Jobs는 "골격(skeleton/code)"을 제공하고, 고객사는 "값(values/infra)"을 채운다.

| 영역 | J-Jobs (skeleton) | 고객사 (values/infra) |
|------|:---:|:---:|
| `awscli` / `kubectl` 컨테이너 포함 | ✅ | — |
| Agent 전용 ServiceAccount manifest (`jjobs-rbac.yaml`) | ✅ | — |
| IRSA annotation placeholder (`eks.amazonaws.com/role-arn`) | ✅ | IAM Role ARN 값 |
| StatefulSet `serviceAccountName` 바인딩 | ✅ | — |
| `KUBE_CONTEXTS` 환경변수 contract 및 entrypoint 부트스트랩 | ✅ | 변수 값 |
| `kubectl` 호출 시 `--context` 플래그 주입 (K8sJobBatchlet) | ✅ | — |
| IRSA IAM Role 생성, Trust policy, IAM permission | — | ✅ |
| 대상 EKS Access Entry 또는 `aws-auth` ConfigMap 매핑 | — | ✅ |
| 대상 클러스터 API endpoint 네트워크 도달성 (SG, routing, VPC peering, TGW, VPC endpoint) | — | ✅ |
| 실제 cluster name, region, role ARN, alias 값 제공 | — | ✅ |

> J-Jobs는 인증 방식(IRSA / kubeconfig Secret / 정적 자격증명 등)을 **선택하거나 검증하지 않는다**. OS/kubectl 단에서 인증되면 동작한다. 다중 계정 EKS 환경에서는 아래 IRSA + AssumeRole 흐름을 권장한다.

### 권장 인증 흐름 (IRSA + cross-account AssumeRole)
...
4. **`KUBE_CONTEXTS` 등록**: StatefulSet env에 아래 스키마로 클러스터 정보를 주입하면 entrypoint가 Pod 부팅 시 `aws eks update-kubeconfig` 를 반복 호출하여 context를 자동 등록한다.
5. **메타데이터 매핑(운영)**: J-Jobs 관리 화면에서 동일 alias를 K8sJob에 매핑한다(이 부분은 별도 기능 plan).
...
| Key | 필수 | 설명 |
|---|:---:|---|
| `alias` | ✅ | kubectl context 이름. K8sJob 메타데이터에 동일 alias로 매핑한다. |
| `provider` |  | reserved. 현재 `eks` 만 구현. 미지정 시 `eks` 로 처리. `gke`/`aks` 는 향후 확장 예정. |
| `cluster` | ✅ | 대상 EKS 클러스터 이름. |
| `region` |  | 대상 클러스터의 AWS region. 다중 region 운영 시 항목별 명시 권장. |
| `roleArn` |  | cross-account 시나리오에서 AssumeRole 대상 IAM Role ARN. 단일 계정이면 생략. |
...
`AWS_PAGER=""` 는 이미지 자체에 박혀 있어 별도 설정 불필요.
...
kubectl --context=prod-b get pods -n <target-namespace>
# → 대상 클러스터의 Pod 목록이 조회되어야 한다.
```
...
- K8s API endpoint 도달성, RBAC, 신뢰관계, IAM 정책은 J-Jobs 보장 범위가 아니며 고객사 인프라 영역이다.